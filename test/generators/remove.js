...
const { directoryExists } = require('../../utils/dir')
...
describe('Remove Generator', () => {
    ...
    `
			...
			.then(() => {
				assert.ok(
					gen.spawnCommandSync.calledWith('docker', ['rmi', 'test_service'])
				)
				assert.ok(
					gen.log.calledWith(chalk.green('Bam! Removed docker images!'))
				)
			})
	})
})
