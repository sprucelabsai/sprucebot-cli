...
async function prompt (options) {
  ...
  if (!path.isAbsolute(values.installPath)) {
    throw new Error(`Woops, I can only install in an absolute installPath. You supplied ${values.installPath}`)
  }
  return values
}
...
async function cloneRepo (repo, localPath) {
  ...
  if (exists) {
    console.log(`Oh snap, looks like you already installed something at ${localPath}! Skipping for now.`)
  } else {
...
module.exports = async function init (localPath, options) {
  ...
  if (cliPath === process.cwd()) {
    console.error(chalk.bold.red('You cannot run `sprucebot platform init` from inside the sprucebot-cli directory.'))
  ...
  const packageFrom = path.resolve(promptValues.installPath, './dev-services/package.json')
  ...
  hostile.get(false, (err, lines) => {
    ...
    if (
      ...
      console.log(chalk.green('Heck yeah! I double checked and everything looks good.'))
      console.log(chalk.yellow(`Run \`cd ${dir} && sprucebot platform start\`  🌲 🤖`))
    }
  })
}
