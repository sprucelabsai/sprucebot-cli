...
module.exports = async function version (installPath = process.cwd(), options) {
  ...
  const repositories = Object.keys(repositoriesConfig).reduce((repos, repo) => ({
    ...repos,
    [repo]: {
  ...
  for (let repo of Object.keys(repositories)) {
    ...
    if (!versions.length) {
      ...
      console.log(chalk.yellow(`Oops, the ${chalk.underline(repositories[repo].name)} repository doesn't have any available versions`))
  ...
  return inquirer.prompt(prompts).then(async (answers) => {
    for (let repo of Object.keys(repositories)) {
      console.log(`Checkout out ${repositories[repo].name}:${answers[`${repo}Version`]}...`)
      await repositories[repo].repository.checkoutBranch(answers[`${repo}Version`])
      console.log(`Finished ${repositories[repo].name}:${answers[`${repo}Version`]}`)
    }
  })
}
